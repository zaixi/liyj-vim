...
then
	...
	then
		...
		echo "\nYouCompleteMe安装不完全,请打开vim重新安装YouCompleteMe\n"
	fi
else
	if test -e $HOME/.vim/plug/repos/github.com/Shougo/dein.vim
	then
		if test -e $HOME/.vim/colors_back
		then
			echo "\nYouCompleteMe未安装,请打开vim重新安装YouCompleteMe\n"
			exit
		fi
	else
		#读取系统类型
